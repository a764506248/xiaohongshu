...
export interface PublishJob { id:string; channel_variant_id:string; channel_account_id:string; idempotency_key:string; approval_status:string; status:string; scheduled_at:string|null; published_at:string|null; external_post_id:string|null; retry_count:number; max_retries:number; error_message:string|null; created_at:string; updated_at:string }
...
export interface AnalyticsSummary { published_posts:number; total_views:number; total_interactions:number; average_score:number; model_calls:number; estimated_model_cost:number; top_signals:PreferenceSignal[] }
